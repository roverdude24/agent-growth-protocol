...
Do **not** duplicate active ops skills here; those live in the Ops lane only.
...
1. Snapshot the current skill tree before any changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase0_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`
...
5. Snapshot before starting Phase 1 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase1_core_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`

### Phase 2 — review lane
1. Move review / audit skills immediately after Core so verification is available for later migrations.
2. Promote or materialize `audit-plan-and-execution` first, then validate the review lane outputs.
3. Confirm verification outputs are strict and evidence-based before continuing.
4. Snapshot before starting Phase 2 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase2_review_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`
...
4. Snapshot before starting Phase 3 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase3_code_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`

### Phase 4 — creative lane
Snapshot before starting Phase 4 changes:
- `SNAPSHOT_DIR="$HOME/.hermes_backup_phase4_creative_$(date +%Y%m%d_%H%M%S)"`
- `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
- `test -d "$SNAPSHOT_DIR"`
...
3. Snapshot before starting Phase 5 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase5_ops_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`
...
3. Snapshot before starting Phase 6 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase6_personal_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`
...
3. Snapshot before starting Phase 7 changes:
   - `SNAPSHOT_DIR="$HOME/.hermes_backup_phase7_retire_$(date +%Y%m%d_%H%M%S)"`
   - `cp -r "$HOME/.hermes" "$SNAPSHOT_DIR"`
   - `test -d "$SNAPSHOT_DIR"`
...
- restore just that skill from the latest phase snapshot
...
- verify the family restore with:
  - `test -f "$HOME/.hermes/skills/<family>/SKILL.md"`
  - `test -d "$HOME/.hermes/skills/<family>/skills"`
  - `LATEST_SNAPSHOT="$(ls -dt "$HOME"/.hermes_backup_* | head -n 1)"`
  - `diff -qr "$LATEST_SNAPSHOT" "$HOME/.hermes"`

### If a lane fails
- revert the whole lane to the previous working version
- keep the other lanes intact
- verify the lane restore with:
  - `test -d "$HOME/.hermes/skills/<lane>"`
  - `test -f "$HOME/.hermes/skills/<lane>/<primary-skill>/SKILL.md"`
  - `LATEST_SNAPSHOT="$(ls -dt "$HOME"/.hermes_backup_* | head -n 1)"`
  - `diff -qr "$LATEST_SNAPSHOT" "$HOME/.hermes"`
...
- [ ] `brainstorming` is listed as on-demand
- [ ] Active ops skills are not duplicated in personal utilities
- [ ] `link-plus-library` appears only once
...
### Safety and rollback
- [ ] Snapshot exists before every migration phase
- [ ] Each lane can be restored independently
- [ ] Family roots are restored before their subskills
- [ ] Rollback verification commands pass after restore
